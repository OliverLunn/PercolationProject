...
S = np.loadtxt('data\square lattice average size data.txt')
...
S1 = np.loadtxt('data\Triangular lattice average size.txt')
for j in range(len(sizes)):
...
ax1.set_ylabel('$S(p,L)$',fontsize="20")
...
ax2.set_ylabel('$S(p,L)$',fontsize="20")
ax2.text(0.3,330,'B',fontsize=20,color ='black')
